...
try{records=JSON.parse(localStorage.getItem('sccSpinWinPlayersV5')||'[]')}catch(e){records=[]}
function buildSlices(){slices=[];PRIZES.forEach(p=>{for(let i=0;i<p.qty;i++)slices.push({...p})});
  // spread prizes for visual balance
  slices=[slices[0],slices[15],slices[1],slices[22],slices[2],slices[27],slices[3],slices[16],slices[4],slices[23],slices[5],slices[28],slices[6],slices[17],slices[7],slices[24],slices[8],slices[18],slices[9],slices[25],slices[10],slices[19],slices[11],slices[26],slices[12],slices[20],slices[13],slices[21],slices[14],slices[29]];
}
buildSlices();
...
function showWinner(prize){winnerName.textContent=currentPlayer?.name||'Player';prizeName.textContent=prize.name;winMascot.src=mascots[Math.floor(Math.random()*mascots.length)];modal.classList.remove('hidden');confetti(); if(currentPlayer){currentPlayer.prize=prize.name;currentPlayer.time=new Date().toLocaleString();records.push(currentPlayer);localStorage.setItem('sccSpinWinPlayersV5',JSON.stringify(records));}}
...
spinBtn.addEventListener('click',spin);$('#awesomeBtn').onclick=()=>modal.classList.add('hidden');$('#nextBtn').onclick=()=>{modal.classList.add('hidden');game.classList.add('hidden');$('#intro').classList.remove('hidden');stage.classList.add('intro-mode');stage.classList.remove('game-mode');form.reset();currentPlayer=null;rotation=0;canvas.style.transform='rotate(0deg)'};
...
$('#resetDataBtn').onclick=()=>{if(confirm('Reset all saved player data?')){records=[];localStorage.removeItem('sccSpinWinPlayersV5')}};
// single day/night controller
let night=false;setInterval(()=>{night=!night;stage.classList.toggle('night',night)},22000);
// simple generated cute loop + effects
let audioCtx=null,bgmTimer=null,musicOn=false;function ac(){return audioCtx||(audioCtx=new (window.AudioContext||window.webkitAudioContext)())}
...
function startMusic(){if(musicOn)return;musicOn=true;const notes=[523,659,784,659,587,698,880,698];let i=0;bgmTimer=setInterval(()=>{tone(notes[i++%notes.length],.22,'triangle',.025)},360)}
$('#musicBtn').onclick=()=>{musicOn?(clearInterval(bgmTimer),musicOn=false):startMusic()};document.addEventListener('pointerdown',()=>{if(!musicOn)startMusic()},{once:true});
