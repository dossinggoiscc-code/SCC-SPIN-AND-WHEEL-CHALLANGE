(() => {
  ...
  const prizes = [
    { name: 'YONO', color: '#0057b7' },
    { name: 'HAND FAN', color: '#f47a00' },
    { name: 'BATTERY FAN', color: '#c90024' },
    { name: 'BACK PACK', color: '#078225' },
    { name: 'MYSTERY GIFT', color: '#c60062' }
  ];
  const wheelSegments = [];
  const pattern = ['YONO','HAND FAN','BATTERY FAN','YONO','HAND FAN','BACK PACK','BATTERY FAN','YONO','HAND FAN','YONO','BATTERY FAN','HAND FAN','BACK PACK','YONO','MYSTERY GIFT','HAND FAN','BATTERY FAN','YONO','HAND FAN','YONO','BACK PACK','BATTERY FAN','HAND FAN','YONO','BATTERY FAN','HAND FAN','BACK PACK','YONO','HAND FAN','BATTERY FAN'];
  pattern.forEach(name => wheelSegments.push(prizes.find(p => p.name === name)));
  ...
  function drawWheel(){
    const w = canvas.width, h = canvas.height, r = w/2, outer = r*0.48;
    ...
    for(let i=0;i<segCount;i++){
      ...
      const grad = ctx.createRadialGradient(0,0,outer*.15,0,0,outer);
      grad.addColorStop(0, '#fff6b0'); grad.addColorStop(.18, p.color); grad.addColorStop(1, shade(p.color,-36));
      ctx.beginPath(); ctx.moveTo(0,0); ctx.arc(0,0,outer,a0,a1); ctx.closePath(); ctx.fillStyle=grad; ctx.fill();
      ctx.strokeStyle = '#ffdf5d'; ctx.lineWidth = 8; ctx.stroke();
      ctx.save(); ctx.rotate(a0+arc/2); ctx.textAlign='right'; ctx.textBaseline='middle'; ctx.fillStyle='#fff'; ctx.shadowColor='rgba(0,0,0,.55)'; ctx.shadowBlur=7;
      ctx.font = '900 38px Fredoka, Arial';
      const label = p.name === 'MYSTERY GIFT' ? '🎁' : p.name;
      ctx.fillText(label, outer-45, 0);
      ctx.restore();
    }
    ctx.beginPath(); ctx.arc(0,0,outer*.25,0,Math.PI*2); ctx.fillStyle='rgba(255,245,150,.35)'; ctx.fill();
    ctx.restore();
  }
  ...
  function spin(){
    ...
    const targetAngle = (segCount - targetIndex) * (360/segCount) + (360/segCount)/2;
    const extra = 360 * (6 + Math.floor(Math.random()*3));
    rotation = rotation + extra + targetAngle - (rotation % 360);
